...
public class ReportMonthAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        name.setText(KKMoneyUtil.GetMonthShort((int) highestMonthExpense.get(position + 1)[1] + 1) + " " + year + KKMoneyUtil.getInstance().GetPurePercentString(highestMonthExpense.get(position + 1)[4] * 100));
        expense.setText(KKMoneyUtil.getInstance().GetInMoney((int) highestMonthExpense.get(position + 1)[3]));
        records.setText(KKMoneyUtil.getInstance().GetInRecords((int) highestMonthExpense.get(position + 1)[5]));
